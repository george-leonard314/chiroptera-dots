    ...
    def save_ca(self) -> None:
        ...
        for index, cert_pem in enumerate(certs):
            with open(os.path.join(ca_dir, f'ca-{index}.pem'), 'w') as f:
                f.write(cert_pem + "\n")
        # ChiropteraOS: always point the profile at one PEM bundle (ca-cert),
        # never at the rehashed directory (ca-path). NetworkManager 1.58+
        # rejects ca-path on private connections (connection.permissions set),
        # and activation fails with 'supplicant-config-failed'.
        self.rehash = False
...
Config.instname = "Fontys Hogescholen"
Config.profilename = "Fontys Studenten en Medewerkers"
Config.url = "your local eduroam® support page"
Config.email = "it-servicedesk@fontys.nl"
Config.title = "eduroam CAT"
Config.server_match = "wifi.fontys.nl"
...
Config.user_realm = "fontys.nl"
Config.ssids = ['eduroam']
Config.del_ssids = []
Config.servers = ['DNS:wifi.fontys.nl']
Config.servers_cn = 'wifi.fontys.nl'
